document.addEventListener('DOMContentLoaded', () => {
    ...
    const temperatureTableBodyElement = document.querySelector('#temperature-table tbody');
    ...
    const generateRandomData = (minutes, interval = 10) => {
        ...
        for (let i = minutes; i >= 0; i--) {
            ...
            const value = generateRandomValue(-10, 30); // Adjust the range as needed
    ...
    temperatureValueElement.textContent = generateRandomValue(-10, 30);
    ...
    displayDataInTable(dataPast1Hour, temperatureTableBodyElement);
    ...
    const updateChart = (data) => {
        ...
        if (window.temperatureChart) {
            window.temperatureChart.destroy(); // Destroy the existing chart if it exists
        }
...
const downloadPdf = (data) => {
    const pdf = new jsPDF();
    pdf.autoTable({ html: '#temperature-table' });
    pdf.save('temperature_data.pdf');
};
